...
const getOpengraph = async () => {
  ...
  const res = await $fetch<OgMeta>(
    `/api/meta?url=${encodeURIComponent('https://www.youtube.com/watch?v=dRGgYvF0IsY')}`
  )
  if (res) {
    ogItem.value = res
  }
  ...
  <BasePage :full="false">
    <BaseCard title="Fetch data">
      <q-card-section class="q-gutter-md">
        <BaseTextHeader title="AsyncData" />
        ...
        <BaseTextHeader title="Internal Server API" />
        ...
        <BaseTextHeader title="Retrieve og: and other meta attributes from a resource" />
        <BaseButton
          outline
          :loading="ogLoading"
          label="Fetch Opengraph detail"
          @click="getOpengraph"
        />
