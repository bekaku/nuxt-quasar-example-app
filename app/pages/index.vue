...
const dashBaordStatisticItems = ref<LabelValue<string>[]>([])
const dashBaordRecentSalseItems = ref<LabelValue<string>[]>([])
const dashboardChartData = ref<ISeriresCategories>()
const dashboardHeroItems = ref<LabelValue<string>[]>([])
const dashboardSparkLineItems = ref<any[]>([])
onMounted(async () => {
  console.log('onMounted')
  dashBaordStatisticItems.value = await $fetch<LabelValue<string>[]>(
    '/api/mock/dashboard/dashBaordStatisticItems'
  )

  dashBaordRecentSalseItems.value = await $fetch<LabelValue<string>[]>(
    '/api/mock/dashboard/dashBaordRecentSalseItems'
  )

  dashboardChartData.value = await $fetch<ISeriresCategories>(
    '/api/mock/dashboard/dashboardChartData'
  )
  dashboardHeroItems.value = await $fetch<LabelValue<string>[]>(
    '/api/mock/dashboard/dashboardHeroItems'
  )
  dashboardSparkLineItems.value = await $fetch<any[]>('/api/mock/dashboard/dashboardSparkLineItems')
        ...
        <div class="col-12 col-md-8 q-px-md">
          <BaseCard title="Overview" :flat="false">
            <q-card-section>
              ...
              <Transition v-if="showChart && dashboardChartData">
                <div>
                  <ChartArea
                    class="q-my-sm"
                    chart-id="chart-bar"
