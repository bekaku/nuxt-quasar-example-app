...
const breadcrumbs = computed<LabelValue<any>[] | undefined>(
  () => getPageMetaByKey('breadcrumbs') as LabelValue<any>[]
)
const tabs = computed<LabelValue<any>[] | undefined>(
  () => getPageMetaByKey('tabs') as LabelValue<any>[]
)
</script>
<template>
  <div
    v-if="(breadcrumbs && breadcrumbs.length > 0) || (tabs && tabs.length > 0)"
    class="row q-mt-lg"
  >
    <div class="col">
      ...
      <div class="block" v-if="tabs && tabs.length > 0">
