...
const { item, dense = true, iconSize = '20px', avatarSize = '24px' } = defineProps<{
    item: LabelValue<T>;
    darkText?: string;
    lightText?: string;
    iconSize?: string;
...
<template>
    <q-item v-bind="$attrs" :to="item.to" :clickable="item.to != undefined" :dense
        ...
        <q-item-section v-if="item.avatar || item.icon" side>
            ...
            <template v-else>
                <q-icon :name="item.icon" :class="{'q-text-black' : !isActive, 'active-menu-link': isActive}" :size="item.iconSize || iconSize" :color="item.color" />
            </template>
        </q-item-section>
        <q-item-section>
            ...
            <q-item-label v-if="item?.description" caption lines="1" :class="{ 'active-menu-link-caption': isActive, 'q-pb-xs': dense }">
                {{ item.description }}
            </q-item-label>
        </q-item-section>
