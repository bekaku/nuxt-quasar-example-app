...
const options = (date: string) => {
    if (minDate && maxDate) {
        return (
            date >= minDate.replaceAll('-', '/') &&
            date <= maxDate.replaceAll('-', '/')
        );
    } else if (minDate) {
...
<template>
    <template v-if="!range">
        <q-date v-model="modelValue" mask="YYYY-MM-DD" :color today-btn :first-day-of-week="0"
            :locale="datePickerLocale" :options="dateList.length > 0 ? limitDates : options"
    ...
    <template v-else>
        <q-date v-model="modelValueRange" range mask="YYYY-MM-DD" :color today-btn :first-day-of-week="0"
            :locale="datePickerLocale" :options="dateList.length > 0 ? limitDates : options"
