...
<template>
    <div class="row q-gutter-x-sm justify-center">
        <q-input v-for="i in length" v-bind="$attrs" :ref="(el) => updateFieldRef(el, i - 1)" :key="i"
            v-model="fieldValues[i - 1]" maxlength="1" outlined input-class="text-center" style="width: 6ch"
            @keyup="onKeyUp($event, i - 1)" @update:model-value="onUpdate($event, i - 1)">
            <slot />
        </q-input>
    </div>
</template>